...
function get_prompt(statement::String, use_history::Bool, training_mode::Bool)
    # Read base prompt
    base_prompt = read("$(dirname(@__DIR__))/prompts/prompt_base.txt", String)
    ...
    elseif use_history
        ...
        context = read("$(dirname(@__DIR__))/prompts/prompt_context.txt", String)
        if !training_mode
        ...
        ignore_context = read("$(dirname(@__DIR__))/prompts/prompt_ignore_context.txt", String)
